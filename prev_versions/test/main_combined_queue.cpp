...
void decodeTask(void * pvParameters){
  int32_t localCurrentStepSize = 0; 
  while(true){
    Serial.println("Decode Task");
    uint8_t RX_Message_local[8];
    xQueueReceive(msgInQ, RX_Message_local, portMAX_DELAY);
    if((char)RX_Message_local[0] == 'P'){
      localCurrentStepSize = (stepSizes[(int)floor(RX_Message[2]/4)][RX_Message[2]%3])*pow(2,((int)RX_Message[1]-4)) ;
    }
