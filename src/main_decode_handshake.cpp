...
void scanKeysTask(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      keyArray[i] = readCols(i);  //Read selected row
      int j = 0;
      for (uint8_t idx = 1; idx < 9; idx = idx * 2) //Cyle through array to find key press
      {
        if ((keyArray[i] & idx) == 0)
        {
          idxKey = i * 4 + j + 1;
